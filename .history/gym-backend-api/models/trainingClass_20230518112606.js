...
export async function create(trainingClass) {
    ...
    return db.query(
        ...
        [trainingClass.datetime,trainingClass.room_id, trainingClass.activity_id, trainingClass.trainer_user_id]
    ).then(([result]) => {
...
export async function update(trainingClass) {
    return db.query(
        "UPDATE training_classes SET (datetime, room_id, activity_id, trainer_user_id) VALUES (?,?,?,?) WHERE id = ?",
        [trainingClass.datetime,trainingClass.room_id, trainingClass.activity_id, trainingClass.trainer_user_id, trainingClass.id]
    ).then(([result]) => {
