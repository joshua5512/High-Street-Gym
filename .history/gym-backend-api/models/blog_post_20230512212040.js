...
export async function getBlogpostByUserID(userID) {

    const [blogpostsResults] = await db.query(
        "SELECT * FROM blog_posts WHERE user_id = ?", userID
    )
    // Convert the result into a Sighting object
    return await blogpostsResults.map((blogpostResult) =>
        Blogpost(
            blogpostResult.id, blogpostResult.datetime,blogpostResult.user_id, blogpostResult.title, blogpostResult.content
        ))
}
...
export async function update(blogpost) {
    return db.query(
        "UPDATE blog_posts SET datetime = ?, user_id=?, title=?, content=?  WHERE id = ?",
        [blogpost.datetime, blogpost.user_id, blogpost.title, blogpost.content, blogpost.id]
    )
}
