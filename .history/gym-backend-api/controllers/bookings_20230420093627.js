...
bookingController.get("/bookings", validate({ body: getBookingListSchema }), async (req, res) => {
    ...
    const bookings = await models.bookingModel.getAll()
...
bookingController.get(
    ...
    async (req, res) => {
        ...
        const bookings = await models.bookingModel.getByUserID(userID)
...
bookingController.get("/bookings/:id", validate({ params: getBookingByIDSchema }), (req, res) => {
    ...
    models.bookingModel.getByID(bookingID).then(booking => {
        res.status(200).json({
            status: 200,
...
bookingController.post("/bookings/", validate({ body: createBookingSchema }), (req, res) => {
    ...
    models.bookingModel.create(booking).then(booking => {
        res.status(200).json({
            status: 200,
...
bookingController.delete("/bookings/", validate({ body: deleteBookingSchema }), (req, res) => {
    ...
    models.bookingController.deleteByID(bookingID).then(result=>{
