...
function Dashboard() {
    ...
    {user ?
        <div className={`container p-2 mt-16 mx-auto grid grid-cols-1 ${user.role === "admin" || user.role === "trainer" ? "md:grid-cols-2" : "md:grid-cols-1"} gap-2`}>
           ...
           <div className={`rounded-2xl border-2 border-primary w-full md:w-1/2 ${user.role === "admin" || user.role === "trainer" ? "md:col-start-1" : "md:col-start-auto mx-auto"} min-h-16 p-2`}>
