...
export default function Classes() {
  ...
  <div className="container p-2 mx-auto">
    <div className="rounded border-2 border-primary p-2 w-full overflow-x">
      <div className="mb-2">
      ...
      {trainingClasses.length == 0 ? (
        ...
        <div className="flex flex-col space-y-4">
          {filteredTrainingClasses.map((trainingClass) => (
            <div key={trainingClass.id} className="border p-2 rounded flex flex-col space-y-2">
              <div>Activity: {trainingClass.activity.name}</div>
              <div>Room: {trainingClass.room.number}</div>
              <div>Datetime: {trainingClass.date}</div>
              <div>
                Created by: {trainingClass.trainer.firstname + " " + trainingClass.trainer.lastname}
              </div>
              <div>
                <button
