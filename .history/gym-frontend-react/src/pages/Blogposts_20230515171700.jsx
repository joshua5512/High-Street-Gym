          ...
          <div className="hidden md:table md:w-full">
            <div className="table-row-group">
              {blogposts.map((blogpost) => (
                <div key={blogpost.id} className="table-row">
                  <div className="table-cell border px-2 py-1">{blogpost.id}</div>
                  <div className="table-cell border px-2 py-1">
                    {blogpost.user.firstname} {blogpost.user.lastname}
                  </div>
                  <div className="table-cell border px-2 py-1">{blogpost.date}</div>
                  <div className="table-cell border px-2 py-1">{blogpost.title}</div>
                  <div className="table-cell border px-2 py-1 truncate">{blogpost.content}</div>
                  <div className="table-cell border px-2 py-1">
                    {user && (user.id === blogpost.user.id || user.role === "admin") ? (
                      <div>
                        <Link to={`/blogposts/${blogpost.id}`} className="btn btn-primary btn-sm mx-2">
                          Edit
                        </Link>
                        <button className="btn btn-primary btn-sm" onClick={() => handleDeleteBlogpost(blogpost.id)}>
                          Delete
                        </button>
                      </div>
                    ) : (
                      <div>
                        <button className="btn btn-sm mx-2" style={{ backgroundColor: 'gray', color: 'white' }} disabled>
                          Edit
                        </button>
                        <button className="btn btn-sm" style={{ backgroundColor: 'gray', color: 'white' }} disabled>
                          Delete
                        </button>
                      </div>
                    )}
