...
import { getTrainerByRole } from "../../../gym-backend-api/models/user";
    ...
    useEffect(() => {
        getTop(200).then(async trainingClasses => {
            ...
            const trainingClassesWithExtras = await Promise.all(trainingClasses.map(async trainingClass => {
                ...
                const trainer = await getTrainerByRole(trainingClass.trainer_user_id)
                        ...
                        <tbody>
                            {trainingClasses.map(trainingClass =>
                                <tr key={trainingClass.id}>
                                    ...
                                    <td>{trainingClass.trainer_user_id.name}</td>
                                    <td>
                                        <button
