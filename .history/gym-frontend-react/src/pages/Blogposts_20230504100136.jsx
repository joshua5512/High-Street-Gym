...
import { getAllBlogposts, getUserBlogposts } from "../api/blogposts";
...
export default function Blogposts({refreshDependency}) {
    ...
    useEffect(() => {
        getAllBlogposts.then(async blogposts => {

            const blogpostsWithExtras = await Promise.all(blogposts.map(async blogpost => {
                
                const userID = await getByID(blogposts.user_id)
