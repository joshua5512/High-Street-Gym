...
export default function Blogposts({ refreshDependency }) {
  ...
  useEffect(() => {
    getAll(user.authenticationKey).then(async (blogposts) => {
  ...
  const handleDeleteBlogpost = async (blogpostID) => {
    try {
      await deleteByID(blogpostID, user.authenticationKey); // pass the authentication key here
  ...
  return (
    <>
      <Nav />
      <div className="container p-2 mt-16 mx-auto grid grid-cols-1 md:grid-cols-1 gap-16 place-items-center">
        <div className="rounded-2xl border-2 border-primary p-2 w-full md:w-3/4 mx-auto">
          {blogposts.length === 0 ? (
            <Spinner />
          ) : (
