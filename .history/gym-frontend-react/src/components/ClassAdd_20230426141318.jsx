...
import { getAll } from "../../../gym-backend-api/models/user"
...
export function ClassAdd({ onAdded }) {
    ...
    useEffect(() => {
        getAllUses().then(users => setUsers(users))
    }, [])
    ...
    return <div>
        <form className="flex-grow m-4 max-w-2xl" onSubmit={addClass} >
            ...
            <div className="form-control">
                ...
                >
                    <option selected>Pick one</option>
                    {users.map(activity => <option key={user.id} value={user.id}>{user.name} - {user.role}-{user.firstname}-{user.lastname}</option>)}
                </select>
            </div>
