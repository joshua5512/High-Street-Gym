...
export default function Nav() {
  ...
  return (
    <div className="flex flex-col-2 items-center md:flex-row md:items-baseline">
      <a href="/" className="btn btn-ghost normal-case text-xl m-2" onClick={() => setShowMenu(false)}>
        High Street Gym
      </a>
      <div className="navbar flex md:justify-start relative">
        <div className="burger-menu md:hidden absolute top-0 right-0" onClick={toggleMenu}>
          <FaBars />
        </div>
        {showMenu && (
          <ul className="menu md:menu-horizontal px-1 w-full absolute top-12 right-0">
            {user && user.role === "admin" ? (
              <li>
                <Link to="/users">Users</Link>
              </li>
