...
export default function UserClasses({refreshDependency, onClassSelected }) {
  ...
  const handleDeleteClass = async (trainingClassID) => {
    try {
      await deleteClass(trainingClassID, user.authenticationKey); // pass the authentication key here
      setTrainingClasss((prevTrainingClasses) =>
        prevTrainingClasses.filter((trainingClass) => trainingClass.id !== trainingClassID)
      );
    } catch (error) {
              ...
              {user.role === "trainer" || user.role === "admin" ? (
                <>
                  <button
                    ...
                    onClick={async () => {
                      const response = await deleteClass({ id: trainingClassID }, user.authenticationKey);
                      if (response.status === 200) {
                        // Filter out the deleted class from state
                        setTrainingClasses(prevClasses => prevClasses.filter(c => c.id !== trainingClass.id));
                      } else {
