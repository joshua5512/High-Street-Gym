...
export default function Nav() {
  ...
  // Determine the name of the user to display in the nav bar
  const userName = user ? user.firstname + " " + user.lastname : "User";
  const userInitials = user ? userinitial(userName) : "U";
    ...
    <div className="flex flex-col items-center md:flex-row md:items-baseline">
      ...
      <div className="navbar flex-col md:flex-row md:justify-between w-full">
        ...
        <ul className={`menu md:menu-horizontal px-1 w-full  ${showMenu ? 'block' : 'hidden'} md:flex md:justify-end`}>
          <li>
            <div className="icon bg-gray-500 text-white rounded-full w-8 h-8 flex items-center justify-center mr-2">{user.firstname.userInitials}</div>
