...
activityController.get("/activities/:id", 
// [auth(["admin", "trainer"]),
validate({ params: getActivityByIDSchema }), (req, res) => {
...
activityController.post("/activities/",[auth(["admin", "trainer"]),  validate({ body: createActivitySchema })], (req, res) => {
...
activityController.delete("/activities/", 
[auth(["admin", "trainer"]),
validate({ body: deleteActivitySchema })], (req, res) => {
            ...
            .then(data => {
                ...
                if (operation == "insert") {
                    Promise.all(activitiesData.map((activityData) => {
                        const activityModel = Activity(null, activityData.name.toString(),
                         activityData.description.toString(),activityData.duration.toString()
                         )
